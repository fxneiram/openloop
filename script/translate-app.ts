...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".openloop/glossary/zh-cn.md"
  if (locale === "zht") return ".openloop/glossary/zh-tw.md"
  return `.openloop/glossary/${locale}.md`
}
...
async function translate(
  ...
  env.OPENLOOP_DISABLE_PROJECT_CONFIG = "1"
  env.OPENLOOP_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
...
function isolatedEnvironment() {
  ...
  delete env.OPENLOOP_CONFIG
  delete env.OPENLOOP_CONFIG_DIR
  delete env.OPENLOOP_CONFIG_CONTENT
  delete env.OPENLOOP_PERMISSION
  delete env.OPENLOOP_AUTO_SHARE
  return env
}
