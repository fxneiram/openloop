...
function globalConfigFile() {
  const candidates = ["openloop.jsonc", "openloop.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.OPENLOOP_CONFIG && !Flag.OPENLOOP_CONFIG_DIR && !Flag.OPENLOOP_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "openloop.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "openloop.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "OPENLOOP_CONFIG_CONTENT") return "local"
        ...
        if (Flag.OPENLOOP_CONFIG) {
          yield* merge(Flag.OPENLOOP_CONFIG, yield* loadFile(Flag.OPENLOOP_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.OPENLOOP_CONFIG })
        }

        if (!Flag.OPENLOOP_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("openloop", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.OPENLOOP_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from OPENLOOP_CONFIG_DIR", { path: Flag.OPENLOOP_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".openloop") || dir === Flag.OPENLOOP_CONFIG_DIR) {
            for (const file of ["openloop.json", "openloop.jsonc"]) {
          ...
          // Auto-discovered plugins under `.openloop/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.OPENLOOP_CONFIG_CONTENT) {
          const source = "OPENLOOP_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.OPENLOOP_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from OPENLOOP_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["OPENLOOP_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("OPENLOOP_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.OPENLOOP_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.OPENLOOP_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("OPENLOOP_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.OPENLOOP_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.OPENLOOP_DISABLE_PRUNE) {
