...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in openloop.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .openloop/ subdirectory too)
  const candidates = [path.join(baseDir, "openloop.json"), path.join(baseDir, "openloop.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".openloop", "openloop.json"), path.join(baseDir, ".openloop", "openloop.jsonc"))
  }
  ...
  // Default to openloop.json if none exist
  return candidates[0]
}
