...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".openloop"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".openloop", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".openloop", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.OPENLOOP_PURE ? [] : pluginOrigins
    if (Flag.OPENLOOP_PURE && pluginOrigins.length) {
    }
