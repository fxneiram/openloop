...
it.instance("plan agent denies edits except .openloop/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".openloop/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".openloop", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.OPENLOOP_TEST_HOME
      process.env.OPENLOOP_TEST_HOME = test.directory
