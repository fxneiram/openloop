...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.OPENLOOP_TEST ?? "")'), {
      env: {
        OPENLOOP_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "OPENLOOP_TEST_SHELL"], {
      shell: true,
      env: {
        OPENLOOP_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("OPENLOOP_TEST_SHELL=ok")
  })
