...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "openloop.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "openloop.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "openloop.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "openloop.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["openloop.json", "openloop.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "openloop.json"),
        path.join(project, "openloop.jsonc"),
      ])
    })
  })
