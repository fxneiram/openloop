...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.OPENLOOP_SERVER_PASSWORD = input?.password
      Flag.OPENLOOP_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".openloop", "skills", "project-rest-skill", "SKILL.md"),
      `---
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(found.data?.data).toContainEqual({ path: "hello.txt", type: "file" })
        expect(found.data?.data).toContainEqual({ path: "openloop.json", type: "file" })
        expect(found.data?.location).toMatchObject({
          directory,
          project: { directory: "/", id: "global" },
          workspaceID,
        })
