...
const originalEnv = {
  OPENLOOP_AUTH_CONTENT: process.env.OPENLOOP_AUTH_CONTENT,
  OPENLOOP_EXPERIMENTAL_WORKSPACES: process.env.OPENLOOP_EXPERIMENTAL_WORKSPACES,
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.OPENLOOP_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.OPENLOOP_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.OPENLOOP_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.OPENLOOP_EXPERIMENTAL_WORKSPACES).toBe("true")
