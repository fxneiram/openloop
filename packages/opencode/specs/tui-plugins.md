...
When `plugin` entries exist in a writable `.openloop` dir or `OPENLOOP_CONFIG_DIR`, OpenCode installs `@opencode-ai/plugin` into that dir and writes:
...
- `--pure` / `OPENLOOP_PURE` skips external TUI plugins only.
