...
const envPath = process.env.OPENLOOP_BIN_PATH
const scriptDir = path.dirname(fs.realpathSync(__filename))
const cached = path.join(scriptDir, ".openloop")
const platform = { darwin: "darwin", linux: "linux", win32: "windows" }[os.platform()] || os.platform()
const arch = { x64: "x64", arm64: "arm64", arm: "arm" }[os.arch()] || os.arch()
const base = "@opencode-ai/cli-" + platform + "-" + arch
const binary = platform === "windows" ? "openloop.exe" : "openloop"
...
if (!resolved) {
  console.error(
    "It seems that your package manager failed to install the right openloop CLI package. Try manually installing " +
