<!--
  ...
  and CUSTOMIZE_OPENLOOP_SKILL_DESCRIPTION). The body below becomes the
  skill's content.
-->
...
- `OPENLOOP_DISABLE_PROJECT_CONFIG=1`: skip the project's local `openloop.json`
  and start from globals only. Run from the project directory, opencode loads,
  the user edits the broken file, then they restart without the flag.
- `OPENLOOP_CONFIG=/path/to/file.json`: load an additional explicit config.
- `OPENLOOP_CONFIG_CONTENT='{"$schema":"https://opencode.ai/config.json"}'`:
  inject inline JSON as a final local-scope merge.
- `OPENLOOP_DISABLE_DEFAULT_PLUGINS=1`: skip default plugins.
- `OPENLOOP_PURE=1`: skip external plugins entirely.
- `OPENLOOP_DISABLE_EXTERNAL_SKILLS=1`,
  `OPENLOOP_DISABLE_CLAUDE_CODE_SKILLS=1`: skip the external skill scans under
  `~/.claude/` and `~/.agents/`.
