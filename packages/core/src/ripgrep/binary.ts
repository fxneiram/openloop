    ...
    Effect.gen(function* () {
      ...
      const extract = Effect.fnUntraced(function* (
        ...
        if (config.extension === "zip") {
          const tar = process.platform === "win32" ? which("tar.exe") : null
          if (tar) {
            ...
            const result = yield* run(shell, [
              ...
              "-NonInteractive",
              "-Command",
              `$global:ProgressPreference = 'SilentlyContinue'; Expand-Archive -LiteralPath '${archive.replaceAll("'", "''")}' -DestinationPath '${dir.replaceAll("'", "''")}' -Force -ErrorAction Stop`,
            ])
            if (result.code !== 0)
              throw new Error(
                result.stderr.trim() || result.stdout.trim() || `ripgrep extraction failed with code ${result.code}`,
