...
| Flag                                        | Short | Description                                                                                                                                                                                                                |
| ------------------------------------------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <nobr><code>{"--path"}</code></nobr>        |       | Directory to write the agent file to (defaults to global or `.openloop/agent` based on the prompt)                                                                                                                         |
...
| <nobr><code>{"--session"}</code></nobr>  | `-s`  | Session ID to continue                                                     |
| <nobr><code>{"--fork"}</code></nobr>     |       | Fork the session when continuing (use with `--continue` or `--session`)    |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Basic auth password (defaults to `OPENLOOP_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `OPENLOOP_SERVER_USERNAME` or `opencode`) |
...
This starts an HTTP server that provides API access to opencode functionality without the TUI interface. Set `OPENLOOP_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
This starts an HTTP server and opens a web browser to access OpenCode through a web interface. Set `OPENLOOP_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
| ------------------------------------- | ------- | ------------------------------------------------- |
| `OPENLOOP_AUTO_SHARE`                 | boolean | Automatically share sessions                      |
| `OPENLOOP_GIT_BASH_PATH`              | string  | Path to Git Bash executable on Windows            |
| `OPENLOOP_CONFIG`                     | string  | Path to config file                               |
| `OPENLOOP_TUI_CONFIG`                 | string  | Path to TUI config file                           |
| `OPENLOOP_CONFIG_DIR`                 | string  | Path to config directory                          |
| `OPENLOOP_CONFIG_CONTENT`             | string  | Inline json config content                        |
| `OPENLOOP_DISABLE_AUTOUPDATE`         | boolean | Disable automatic update checks                   |
| `OPENLOOP_DISABLE_PRUNE`              | boolean | Disable pruning of old data                       |
| `OPENLOOP_DISABLE_TERMINAL_TITLE`     | boolean | Disable automatic terminal title updates          |
| `OPENLOOP_PERMISSION`                 | string  | Inlined json permissions config                   |
| `OPENLOOP_DISABLE_DEFAULT_PLUGINS`    | boolean | Disable default plugins                           |
| `OPENLOOP_DISABLE_LSP_DOWNLOAD`       | boolean | Disable automatic LSP server downloads            |
| `OPENLOOP_ENABLE_EXPERIMENTAL_MODELS` | boolean | Enable experimental models                        |
| `OPENLOOP_DISABLE_AUTOCOMPACT`        | boolean | Disable automatic context compaction              |
| `OPENLOOP_DISABLE_CLAUDE_CODE`        | boolean | Disable reading from `.claude` (prompt + skills)  |
| `OPENLOOP_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Disable reading `~/.claude/CLAUDE.md`             |
| `OPENLOOP_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Disable loading `.claude/skills`                  |
| `OPENLOOP_DISABLE_MODELS_FETCH`       | boolean | Disable fetching models from remote sources       |
| `OPENLOOP_DISABLE_MOUSE`              | boolean | Disable mouse capture in the TUI                  |
| `OPENLOOP_FAKE_VCS`                   | string  | Fake VCS provider for testing purposes            |
| `OPENLOOP_CLIENT`                     | string  | Client identifier (defaults to `cli`)             |
| `OPENLOOP_ENABLE_EXA`                 | boolean | Enable Exa web search tools                       |
| `OPENLOOP_SERVER_PASSWORD`            | string  | Enable basic auth for `serve`/`web`               |
| `OPENLOOP_SERVER_USERNAME`            | string  | Override basic auth username (default `opencode`) |
| `OPENLOOP_MODELS_URL`                 | string  | Custom URL for fetching models configuration      |
...
| Variable                                        | Type    | Description                             |
| ----------------------------------------------- | ------- | --------------------------------------- |
| `OPENLOOP_EXPERIMENTAL`                         | boolean | Enable the experimental umbrella flag   |
| `OPENLOOP_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Enable icon discovery                   |
| `OPENLOOP_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Disable copy on select in TUI           |
| `OPENLOOP_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Default timeout for bash commands in ms |
| `OPENLOOP_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Max output tokens for LLM responses     |
| `OPENLOOP_EXPERIMENTAL_FILEWATCHER`             | boolean | Enable file watcher for entire dir      |
| `OPENLOOP_EXPERIMENTAL_OXFMT`                   | boolean | Enable oxfmt formatter                  |
| `OPENLOOP_EXPERIMENTAL_LSP_TOOL`                | boolean | Enable experimental LSP tool            |
| `OPENLOOP_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Disable file watcher                    |
| `OPENLOOP_EXPERIMENTAL_EXA`                     | boolean | Enable experimental Exa features        |
| `OPENLOOP_EXPERIMENTAL_LSP_TY`                  | boolean | Enable TY LSP for python files          |
| `OPENLOOP_EXPERIMENTAL_PLAN_MODE`               | boolean | Enable plan mode                        |
| `OPENLOOP_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Enable background subagent tasks        |
| `OPENLOOP_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Enable experimental event system        |
| `OPENLOOP_EXPERIMENTAL_NATIVE_LLM`              | boolean | Enable native LLM request path          |
| `OPENLOOP_EXPERIMENTAL_PARALLEL`                | boolean | Enable parallel web search execution    |
| `OPENLOOP_EXPERIMENTAL_SCOUT`                   | boolean | Enable Scout subagent                   |
| `OPENLOOP_EXPERIMENTAL_WORKSPACES`              | boolean | Enable workspace support                |
