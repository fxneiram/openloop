...
Du kannst auch globale Regeln in `~/.config/openloop/AGENTS.md` speichern.
...
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/openloop/AGENTS.md` existiert)
...
2. **Globale Datei** unter `~/.config/openloop/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `openloop.json` oder in `~/.config/openloop/openloop.json` angeben.
...
Empfohlen ist das `instructions`-Feld in der `openloop.json`:
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `openloop.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
