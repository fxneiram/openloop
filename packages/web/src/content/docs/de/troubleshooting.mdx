...
- **macOS/Linux**: `~/.config/openloop/openloop.jsonc` (oder `~/.config/openloop/openloop.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/opencode/openloop.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\opencode\openloop.jsonc`
...
- **Globale Plugins**
  - **macOS/Linux**: `~/.config/openloop/plugins/`
...
Wenn deine `openloop.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `OPENLOOP_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `OPENLOOP_PORT` zurueck (oder waehle einen freien Port) und starte neu.
