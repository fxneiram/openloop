...
:::note
Dieses Tool ist nur verfuegbar, wenn `OPENLOOP_EXPERIMENTAL_LSP_TOOL=true` (oder `OPENLOOP_EXPERIMENTAL=true`) gesetzt ist.
:::
...
:::note
Dieses Tool ist nur verfuegbar, wenn der OpenCode-Provider genutzt wird oder die Umgebungsvariable `OPENLOOP_ENABLE_EXA` auf einen 'truthy' Wert (z. B. `true` oder `1`) gesetzt ist.
