...
2. **Globale Konfiguration** (`~/.config/openloop/openloop.json`) – Benutzereinstellungen
3. **Benutzerdefinierte Konfiguration** (`OPENLOOP_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`openloop.json` im Projekt) – projektspezifische Einstellungen
5. **`.openloop` Verzeichnisse** – Agenten, Befehle, Plugins
6. **Inline-Konfiguration** (`OPENLOOP_CONFIG_CONTENT` env var) – Laufzeitüberschreibungen
...
:::note
Die Verzeichnisse `.openloop` und `~/.config/openloop` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Platzieren Sie Ihre globale OpenCode-Konfiguration in `~/.config/openloop/openloop.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Fügen Sie `openloop.json` in Ihrem Projektstamm hinzu. Die Projektkonfiguration hat unter den Standardkonfigurationsdateien die höchste Priorität – sie überschreibt sowohl globale als auch Remote-Konfigurationen.
...
Geben Sie mithilfe der Umgebungsvariablen `OPENLOOP_CONFIG` einen benutzerdefinierten Konfigurationsdateipfad an.
...
Geben Sie mit `OPENLOOP_CONFIG_DIR` ein benutzerdefiniertes Konfigurationsverzeichnis an.
Umgebungsvariable. Dieses Verzeichnis wird nach Agenten, Befehlen,
Modi und Plugins genau wie das Standardverzeichnis `.openloop` und sollten
...
Das benutzerdefinierte Verzeichnis wird nach den Verzeichnissen global config und `.openloop` geladen, sodass es deren Einstellungen **überschreiben** kann.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/openloop/agents/` oder `.openloop/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/openloop/commands/` oder `.openloop/commands/` definieren. [Mehr erfahren](/docs/commands).
...
Platzieren Sie Ihre Plugin-Dateien in `.openloop/plugins/` oder `~/.config/openloop/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
