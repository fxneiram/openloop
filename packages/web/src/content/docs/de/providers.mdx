...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `openloop.json`:
...
`openloop.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
4. Erstellen oder aktualisieren Sie Ihre `openloop.json`-Datei in Ihrem Projektverzeichnis:
