...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/openloop/themes/*.json` oder `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.openloop/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.openloop/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/openloop/themes
vim ~/.config/openloop/themes/my-theme.json
```

Und fuer projektspezifische Themes:

```bash no-frame
mkdir -p .openloop/themes
vim .openloop/themes/my-theme.json
```
