...
2. **Configuration globale** (`~/.config/openloop/openloop.json`) - préférences utilisateur
3. **Configuration personnalisée** (`OPENLOOP_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`openloop.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.openloop`** - agents, commandes, plugins
6. **Configuration en ligne** (`OPENLOOP_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.openloop` et `~/.config/openloop` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Placez votre configuration globale OpenCode dans `~/.config/openloop/openloop.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/openloop/tui.json`.
...
Ajoutez `openloop.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `OPENLOOP_CONFIG`.

```bash
export OPENLOOP_CONFIG=/path/to/my/custom-config.json
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `OPENLOOP_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.openloop`, et devrait suivre la même structure.

```bash
export OPENLOOP_CONFIG_DIR=/path/to/my/config-directory
opencode run "Hello world"
```

Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.openloop`, il **peut donc remplacer** leurs paramètres.
...
Utilisez `OPENLOOP_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `openloop.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/openloop/agents/` ou `.openloop/agents/`. [En savoir plus ici](/docs/agents).
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/openloop/commands/` ou `.openloop/commands/`. [En savoir plus ici](/docs/commands).
...
Placez les fichiers du plugin dans `.openloop/plugins/` ou `~/.config/openloop/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
