...
:::note
Cet outil n'est disponible que lorsque `OPENLOOP_EXPERIMENTAL_LSP_TOOL=true` (ou `OPENLOOP_EXPERIMENTAL=true`).
:::
...
:::note
Cet outil n'est disponible que lors de l'utilisation du fournisseur OpenCode ou lorsque la variable d'environnement `OPENLOOP_ENABLE_EXA` est définie sur une valeur véridique (par exemple, `true` ou `1`).
