...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/openloop/themes/*.json` ou `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.openloop/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.openloop/themes/*.json`
