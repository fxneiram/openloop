...
Vous pouvez également avoir des règles globales dans un fichier `~/.config/openloop/AGENTS.md`. Ceci est appliqué à toutes les sessions opencode.
...
- **Règles globales** : `~/.claude/CLAUDE.md` (utilisé si aucun `~/.config/openloop/AGENTS.md` n'existe)
...
```bash
export OPENLOOP_DISABLE_CLAUDE_CODE=1        # Disable all .claude support
export OPENLOOP_DISABLE_CLAUDE_CODE_PROMPT=1 # Disable only ~/.claude/CLAUDE.md
export OPENLOOP_DISABLE_CLAUDE_CODE_SKILLS=1 # Disable only .claude/skills
```
...
2. **Fichier global** à `~/.config/openloop/AGENTS.md`
3. **Fichier Claude Code** à `~/.claude/CLAUDE.md` (sauf si désactivé)

Le premier fichier correspondant gagne dans chaque catégorie. Par exemple, si vous possédez à la fois `AGENTS.md` et `CLAUDE.md`, seul `AGENTS.md` est utilisé. De même, `~/.config/openloop/AGENTS.md` a priorité sur `~/.claude/CLAUDE.md`.

---

## Instructions personnalisées

Vous pouvez spécifier des fichiers d'instructions personnalisés dans votre `openloop.json` ou dans le `~/.config/openloop/openloop.json` global. Cela vous permet, à vous et à votre équipe, de réutiliser les règles existantes plutôt que d'avoir à les dupliquer dans AGENTS.md.
...
### Utilisation de openloop.json

L'approche recommandée consiste à utiliser le champ `instructions` dans `openloop.json` :
...
:::tip
Pour les monorepos ou les projets avec des normes partagées, l'utilisation de `openloop.json` avec des modèles globaux (comme `packages/*/AGENTS.md`) est plus maintenable que les instructions manuelles.
:::
