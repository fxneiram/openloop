...
- **macOS/Linux** : `~/.config/openloop/openloop.jsonc` (ou `~/.config/openloop/openloop.json`)
- **macOS/Linux** (anciennes installations) : `~/.local/share/opencode/openloop.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\opencode\openloop.jsonc`
...
Si votre `openloop.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `OPENLOOP_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `OPENLOOP_PORT` (ou choisissez un port libre) et redémarrez.
