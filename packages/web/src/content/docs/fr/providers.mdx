...
Pour une configuration spécifique au projet ou persistante, utilisez `openloop.json` :
...
Fichier `openloop.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `openloop.json` :
...
4. Créez ou mettez à jour votre fichier `openloop.json` dans le répertoire de votre projet :
