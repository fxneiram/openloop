...
-**macOS/리눅스**: `~/.config/openloop/openloop.jsonc` (또는 `~/.config/openloop/openloop.json`) -**macOS/Linux** (외부 설치): `~/.local/share/opencode/openloop.jsonc`

- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.config\opencode\openloop.jsonc`
...
- **글로벌 플러그인** -**macOS/리눅스**: `~/.config/openloop/plugins/`
...
`openloop.json(c)`가 `server` 섹션을 포함하면 일시적으로 제거하고 데스크톱 앱을 다시 시작합니다.

### 환경 변수 확인

`OPENLOOP_PORT`가 있는 경우, 데스크탑 앱은 로컬 서버의 포트를 사용하려고 합니다.

- `OPENLOOP_PORT` (또는 무료 포트를 선택) 및 재시작.
