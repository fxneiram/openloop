...
2. **사용자 설정 디렉토리 (User config directory)** - `~/.config/openloop/themes/*.json` 또는 `$XDG_CONFIG_HOME/opencode/themes/*.json`에 정의됩니다.
3. **프로젝트 루트 디렉토리 (Project root directory)** - `<project-root>/.openloop/themes/*.json`에 정의됩니다.
4. **현재 작업 디렉토리 (Current working directory)** - `./.openloop/themes/*.json`에 정의됩니다.
