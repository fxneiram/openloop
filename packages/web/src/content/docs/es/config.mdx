...
2. **Configuración global** (`~/.config/openloop/openloop.json`) - preferencias del usuario
3. **Configuración personalizada** (`OPENLOOP_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`openloop.json` en el proyecto): configuración específica del proyecto
5. Directorios **`.openloop`** - agentes, comandos, complementos
6. **Configuración en línea** (`OPENLOOP_CONFIG_CONTENT` env var): anulaciones del tiempo de ejecución
...
:::note
Los directorios `.openloop` y `~/.config/openloop` usan **nombres en plural** para los subdirectorios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` y `themes/`. También se admiten nombres singulares (por ejemplo, `agent/`) para compatibilidad con versiones anteriores.
:::
...
Coloque su configuración global OpenCode en `~/.config/openloop/openloop.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Agregue `openloop.json` en la raíz de su proyecto. La configuración del proyecto tiene la mayor prioridad entre los archivos de configuración estándar: anula las configuraciones globales y remotas.
...
Especifique una ruta de archivo de configuración personalizada utilizando la variable de entorno `OPENLOOP_CONFIG`.
...
Especifique un directorio de configuración personalizado usando `OPENLOOP_CONFIG_DIR`
variable de entorno. En este directorio se buscarán agentes, comandos,
modos y complementos como el directorio estándar `.openloop`, y debería
...
El directorio personalizado se carga después de los directorios global config y `.openloop`, por lo que **puede anular** su configuración.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/openloop/agents/` o `.openloop/agents/`. [Más información aquí](/docs/agents).
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/openloop/commands/` o `.openloop/commands/`. [Más información aquí](/docs/commands).
...
Coloque los archivos de complemento en `.openloop/plugins/` o `~/.config/openloop/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
