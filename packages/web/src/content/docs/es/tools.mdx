...
:::note
Esta herramienta solo está disponible cuando `OPENLOOP_EXPERIMENTAL_LSP_TOOL=true` (o `OPENLOOP_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor OpenCode o cuando la variable de entorno `OPENLOOP_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar OpenCode:

```bash
OPENLOOP_ENABLE_EXA=1 opencode
```
