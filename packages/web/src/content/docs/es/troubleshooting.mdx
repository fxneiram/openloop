...
- **macOS/Linux**: `~/.config/openloop/openloop.jsonc` (o `~/.config/openloop/openloop.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/opencode/openloop.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\opencode\openloop.jsonc`
...
Si su `openloop.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `OPENLOOP_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `OPENLOOP_PORT` (o elija un puerto libre) y reinicie.
