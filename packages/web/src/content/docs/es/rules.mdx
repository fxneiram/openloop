...
También puedes tener reglas globales en un archivo `~/.config/openloop/AGENTS.md`. Esto se aplica en todas las sesiones opencode.
...
- **Reglas globales**: `~/.claude/CLAUDE.md` (se usa si no existe `~/.config/openloop/AGENTS.md`)
...
2. **Archivo global** en `~/.config/openloop/AGENTS.md`
3. **Archivo de Claude Code** en `~/.claude/CLAUDE.md` (a menos que esté deshabilitado)

El primer archivo coincidente gana en cada categoría. Por ejemplo, si tiene `AGENTS.md` y `CLAUDE.md`, solo se usa `AGENTS.md`. De manera similar, `~/.config/openloop/AGENTS.md` tiene prioridad sobre `~/.claude/CLAUDE.md`.

---

## Instrucciones personalizadas

Puede especificar archivos de instrucciones personalizados en su `openloop.json` o el `~/.config/openloop/openloop.json` global. Esto le permite a usted y a su equipo reutilizar las reglas existentes en lugar de tener que duplicarlas en AGENTS.md.
...
El enfoque recomendado es utilizar el campo `instructions` en `openloop.json`:
...
:::tip
Para monorepos o proyectos con estándares compartidos, usar `openloop.json` con patrones globales (como `packages/*/AGENTS.md`) es más fácil de mantener que las instrucciones manuales.
:::
