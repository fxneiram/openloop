...
- **macOS/Linux**: `~/.config/openloop/openloop.jsonc` (lub `~/.config/openloop/openloop.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/opencode/openloop.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\opencode\openloop.jsonc`
...
Jeśli Twój `openloop.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `OPENLOOP_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `OPENLOOP_PORT` (lub wybierz wolny port) i uruchom ponownie.
