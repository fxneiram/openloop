...
2. **Konfiguracja globalna** (`~/.config/openloop/openloop.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `OPENLOOP_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`openloop.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.openloop`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `OPENLOOP_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.openloop` i `~/.config/openloop` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Umieść swoją globalną konfigurację OpenCode w `~/.config/openloop/openloop.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/openloop/tui.json`.
...
Dodaj `openloop.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `OPENLOOP_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `OPENLOOP_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.openloop` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.openloop`, więc **może być łączony** z ich zawartością.
...
Użyj `OPENLOOP_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `openloop.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/openloop/agents/` lub `.openloop/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/openloop/commands/` lub `.openloop/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Umieść pliki wtyczek w `.openloop/plugins/` lub `~/.config/openloop/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
