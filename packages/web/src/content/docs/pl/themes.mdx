...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/openloop/themes/*.json` lub `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.openloop/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.openloop/themes/*.json`
