...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `OPENLOOP_EXPERIMENTAL_LSP_TOOL=true` (lub `OPENLOOP_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy OpenCode lub gdy zmienna środowiskowa `OPENLOOP_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).
