...
Możesz także mieć reguły globalne w pliku `~/.config/openloop/AGENTS.md`. Jest to stosowane we wszystkich sesjach opencode.
...
- **Zasady globalne**: `~/.claude/CLAUDE.md` (używane, jeśli nie istnieje `~/.config/openloop/AGENTS.md`)
...
2. **Plik globalny** w `~/.config/openloop/AGENTS.md`
3. **Plik Claude Code** w `~/.claude/CLAUDE.md` (chyba że wyłączony)

Pierwszy pasujący plik wygrywa w każdej kategorii. Na przykład, jeśli masz zarówno `AGENTS.md`, jak i `CLAUDE.md`, używany jest tylko `AGENTS.md`. Podobnie `~/.config/openloop/AGENTS.md` ma pierwszeństwo przed `~/.claude/CLAUDE.md`.

---

## Instrukcje niestandardowe

Możesz określić niestandardowe pliki instrukcji w swoim `openloop.json` lub globalnym `~/.config/openloop/openloop.json`. Dzięki temu Ty i Twój zespół możecie ponownie wykorzystać istniejące reguły, zamiast duplikować je w pliku AGENTS.md.
...
### Używanie openloop.json

Zalecanym podejściem jest użycie pola `instructions` w `openloop.json`:
...
:::tip
W przypadku monorepo lub projektów ze wspólnymi standardami użycie `openloop.json` ze wzorami globalnymi (takimi jak `packages/*/AGENTS.md`) jest łatwiejsze w utrzymaniu niż instrukcje ręczne.
:::
