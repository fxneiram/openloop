...
:::note
Questo strumento e' disponibile solo quando `OPENLOOP_EXPERIMENTAL_LSP_TOOL=true` (oppure `OPENLOOP_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider OpenCode o quando la variabile d'ambiente `OPENLOOP_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).
