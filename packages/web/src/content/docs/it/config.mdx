...
2. **Config globale** (`~/.config/openloop/openloop.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `OPENLOOP_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`openloop.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.openloop`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `OPENLOOP_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.openloop` e `~/.config/openloop` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Metti la configurazione globale di OpenCode in `~/.config/openloop/openloop.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/openloop/tui.json`.
...
Aggiungi `openloop.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `OPENLOOP_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `OPENLOOP_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.openloop` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.openloop`, quindi **puo sovrascrivere** le loro impostazioni.
...
Usa `OPENLOOP_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `openloop.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi anche definire agenti usando file markdown in `~/.config/openloop/agents/` o `.openloop/agents/`. [Scopri di piu](/docs/agents).
...
Puoi anche definire comandi usando file markdown in `~/.config/openloop/commands/` o `.openloop/commands/`. [Scopri di piu](/docs/commands).
...
Metti i file dei plugin in `.openloop/plugins/` o `~/.config/openloop/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
