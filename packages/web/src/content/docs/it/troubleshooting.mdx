...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/opencode/openloop.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\opencode\openloop.jsonc`
...
Se il tuo `openloop.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `OPENLOOP_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `OPENLOOP_PORT` (o scegli una porta libera) e riavvia.
