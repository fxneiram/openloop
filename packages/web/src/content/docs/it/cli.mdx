...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `OPENLOOP_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENLOOP_SERVER_USERNAME` o `opencode`) |
...
Avvia un server HTTP che espone accesso API alle funzionalità di opencode senza la TUI. Imposta `OPENLOOP_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Avvia un server HTTP e apre un browser per accedere a OpenCode tramite interfaccia web. Imposta `OPENLOOP_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
| ------------------------------------- | ------- | ----------------------------------------------------------- |
| `OPENLOOP_AUTO_SHARE`                 | boolean | Condivide automaticamente le sessioni                       |
| `OPENLOOP_GIT_BASH_PATH`              | string  | Percorso all'eseguibile Git Bash su Windows                 |
| `OPENLOOP_CONFIG`                     | string  | Percorso al file di configurazione                          |
| `OPENLOOP_TUI_CONFIG`                 | string  | Percorso al file di configurazione TUI                      |
| `OPENLOOP_CONFIG_DIR`                 | string  | Percorso alla directory di configurazione                   |
| `OPENLOOP_CONFIG_CONTENT`             | string  | Contenuto JSON di config inline                             |
| `OPENLOOP_DISABLE_AUTOUPDATE`         | boolean | Disabilita i controlli automatici di aggiornamento          |
| `OPENLOOP_DISABLE_PRUNE`              | boolean | Disabilita la potatura dei dati vecchi                      |
| `OPENLOOP_DISABLE_TERMINAL_TITLE`     | boolean | Disabilita aggiornamenti automatici del titolo terminale    |
| `OPENLOOP_PERMISSION`                 | string  | Config permessi JSON inline                                 |
| `OPENLOOP_DISABLE_DEFAULT_PLUGINS`    | boolean | Disabilita i plugin di default                              |
| `OPENLOOP_DISABLE_LSP_DOWNLOAD`       | boolean | Disabilita download automatico dei server LSP               |
| `OPENLOOP_ENABLE_EXPERIMENTAL_MODELS` | boolean | Abilita modelli sperimentali                                |
| `OPENLOOP_DISABLE_AUTOCOMPACT`        | boolean | Disabilita compaction automatica del contesto               |
| `OPENLOOP_DISABLE_CLAUDE_CODE`        | boolean | Disabilita lettura da `.claude` (prompt + skill)            |
| `OPENLOOP_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Disabilita lettura di `~/.claude/CLAUDE.md`                 |
| `OPENLOOP_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Disabilita caricamento di `.claude/skills`                  |
| `OPENLOOP_DISABLE_MODELS_FETCH`       | boolean | Disabilita fetch dei modelli da fonti remote                |
| `OPENLOOP_FAKE_VCS`                   | string  | Provider VCS finto per scopi di test                        |
| `OPENLOOP_CLIENT`                     | string  | Identificatore client (default `cli`)                       |
| `OPENLOOP_ENABLE_EXA`                 | boolean | Abilita gli strumenti di web search Exa                     |
| `OPENLOOP_SERVER_PASSWORD`            | string  | Abilita basic auth per `serve`/`web`                        |
| `OPENLOOP_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `opencode`)     |
| `OPENLOOP_MODELS_URL`                 | string  | URL personalizzato per recuperare la configurazione modelli |
...
| Variabile                                       | Tipo    | Descrizione                                             |
| ----------------------------------------------- | ------- | ------------------------------------------------------- |
| `OPENLOOP_EXPERIMENTAL`                         | boolean | Abilita funzionalità sperimentali sotto il flag globale |
| `OPENLOOP_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Abilita icon discovery                                  |
| `OPENLOOP_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Disabilita copy on select nella TUI                     |
| `OPENLOOP_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Timeout di default per comandi bash in ms               |
| `OPENLOOP_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Massimo token di output per risposte LLM                |
| `OPENLOOP_EXPERIMENTAL_FILEWATCHER`             | boolean | Abilita file watcher per l'intera dir                   |
| `OPENLOOP_EXPERIMENTAL_OXFMT`                   | boolean | Abilita formatter oxfmt                                 |
| `OPENLOOP_EXPERIMENTAL_LSP_TOOL`                | boolean | Abilita strumento LSP sperimentale                      |
| `OPENLOOP_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Disabilita file watcher                                 |
| `OPENLOOP_EXPERIMENTAL_EXA`                     | boolean | Abilita funzionalità Exa sperimentali                   |
| `OPENLOOP_EXPERIMENTAL_LSP_TY`                  | boolean | Abilita TY LSP per i file python                        |
| `OPENLOOP_EXPERIMENTAL_PLAN_MODE`               | boolean | Abilita plan mode                                       |
| `OPENLOOP_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Abilita task subagent in background                     |
| `OPENLOOP_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Abilita sistema eventi sperimentale                     |
| `OPENLOOP_EXPERIMENTAL_NATIVE_LLM`              | boolean | Abilita percorso nativo di richiesta LLM                |
| `OPENLOOP_EXPERIMENTAL_PARALLEL`                | boolean | Abilita esecuzione parallela della ricerca web          |
| `OPENLOOP_EXPERIMENTAL_SCOUT`                   | boolean | Abilita subagent Scout                                  |
| `OPENLOOP_EXPERIMENTAL_WORKSPACES`              | boolean | Abilita supporto workspace                              |
