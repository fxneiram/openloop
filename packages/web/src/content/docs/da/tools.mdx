...
:::note
Dette verktøyet er bare tilgængelig når `OPENLOOP_EXPERIMENTAL_LSP_TOOL=true` (eller `OPENLOOP_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger OpenCode-leverandøren eller når miljøvariabelen `OPENLOOP_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
