...
Du kan også ha globale regler i en `~/.config/openloop/AGENTS.md`-fil. Dette blir brugt på alle opencode-sessioner.
...
- **Globale regler**: `~/.claude/CLAUDE.md` (bruges hvis ingen `~/.config/openloop/AGENTS.md` eksisterer)
...
2. **Global fil** på `~/.config/openloop/AGENTS.md`
3. **Claude Code-fil** på `~/.claude/CLAUDE.md` (med mindre de er deaktivert)

Den første matchende filen vinner i hver kategori. For eksempel, hvis du har både `AGENTS.md` og `CLAUDE.md`, bruges bare `AGENTS.md`. På samme måte har `~/.config/openloop/AGENTS.md` forrang over `~/.claude/CLAUDE.md`.

---

## Brugerdefinerede instruktioner

Du kan spesifisere egendefinerte instruksjonsfiler i din `openloop.json` eller den globale `~/.config/openloop/openloop.json`. Dette lar deg og teamet ditt gjenbruge eksisterende regler i stedet for at måtte duplisere dem til AGENTS.md.
...
### Brug af openloop.json

Den anbefalte tilnærmingen er at bruge feltet `instructions` i `openloop.json`:
...
:::tip
For monorepos eller prosjekter med delte standarder er brug av `openloop.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::
