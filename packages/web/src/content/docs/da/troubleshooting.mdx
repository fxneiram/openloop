...
- **macOS/Linux** (ældre installationer): `~/.local/share/opencode/openloop.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\opencode\openloop.jsonc`
...
Hvis `openloop.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `OPENLOOP_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `OPENLOOP_PORT` (eller vælg en ledig port) og start på ny.
