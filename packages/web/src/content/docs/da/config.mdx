...
2. **Global config** (`~/.config/openloop/openloop.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`OPENLOOP_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`openloop.json` i projekt) - projektspecifikke indstillinger
5. **`.openloop` mapper** - agent, kommandoer, plugins
6. **Inline config** (`OPENLOOP_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.openloop` og `~/.config/openloop` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Placer din globale OpenCode-konfiguration i `~/.config/openloop/openloop.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `openloop.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `OPENLOOP_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `OPENLOOP_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.openloop` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.openloop` mapper, så den **kan tilsidesætte** deres indstillinger.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/openloop/agents/` eller `.openloop/agents/`. [Learn more here](/docs/agents).
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/openloop/commands/` eller `.openloop/commands/`. [Learn more here](/docs/commands).
...
Placer plugin-filer i `.openloop/plugins/` eller `~/.config/openloop/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
