...
:::caution
Hvis `OPENLOOP_SERVER_PASSWORD` ikke er indstillet, vil serveren være usikret. Dette er fint til lokal brug, men bør indstilles til netværksadgang.
:::
...
For at beskytte adgangen skal du angive en adgangskode ved hjælp af `OPENLOOP_SERVER_PASSWORD` miljøvariablen:

```bash
OPENLOOP_SERVER_PASSWORD=secret opencode web
```

Brugernavnet er som standard `opencode`, men kan ændres med `OPENLOOP_SERVER_USERNAME`.
...
Du kan også konfigurere serverindstillinger i din `openloop.json`-konfigurationsfil:
