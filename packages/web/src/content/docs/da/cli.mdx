...
| <nobr><code>{"--session"}</code></nobr>  | `-s` | Sessions-id for at fortsætte                                                      |
| <nobr><code>{"--fork"}</code></nobr>     |      | Forgren sessionen ved fortsættelse (brug med `--continue` eller `--session`)      |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Adgangskode til basic auth (standard: `OPENLOOP_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENLOOP_SERVER_USERNAME` eller `opencode`) |
...
Dette starter en HTTP-server, der giver API-adgang til opencode-funktionalitet uden TUI-grænsefladen. Indstil `OPENLOOP_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
Dette starter en HTTP-server og åbner en webbrowser for at få adgang til OpenCode via en webgrænseflade. Indstil `OPENLOOP_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
| ------------------------------------- | ------- | --------------------------------------------------------------------- |
| `OPENLOOP_AUTO_SHARE`                 | boolean | Del automatisk session                                                |
| `OPENLOOP_GIT_BASH_PATH`              | string  | Sti til Git Bash eksekverbar på Windows                               |
| `OPENLOOP_CONFIG`                     | string  | Sti til konfigurationsfil                                             |
| `OPENLOOP_TUI_CONFIG`                 | string  | Sti til TUI-konfigurationsfil                                         |
| `OPENLOOP_CONFIG_DIR`                 | string  | Sti til konfigurationsmappe                                           |
| `OPENLOOP_CONFIG_CONTENT`             | string  | Indbygget json-konfigurationsindhold                                  |
| `OPENLOOP_DISABLE_AUTOUPDATE`         | boolean | Deaktiver automatisk opdateringskontrol                               |
| `OPENLOOP_DISABLE_PRUNE`              | boolean | Deaktiver beskæring af gamle data                                     |
| `OPENLOOP_DISABLE_TERMINAL_TITLE`     | boolean | Deaktiver automatisk opdatering af terminaltitel                      |
| `OPENLOOP_PERMISSION`                 | string  | Indbygget json-tilladelseskonfiguration                               |
| `OPENLOOP_DISABLE_DEFAULT_PLUGINS`    | boolean | Deaktiver standard plugins                                            |
| `OPENLOOP_DISABLE_LSP_DOWNLOAD`       | boolean | Deaktiver automatisk LSP-serverdownloads                              |
| `OPENLOOP_ENABLE_EXPERIMENTAL_MODELS` | boolean | Aktive eksperimentelle modeller                                       |
| `OPENLOOP_DISABLE_AUTOCOMPACT`        | boolean | Deaktiver automatisk kontekstkomprimering                             |
| `OPENLOOP_DISABLE_CLAUDE_CODE`        | boolean | Deaktiver læsning fra `.claude` (prompt + færdigheder)                |
| `OPENLOOP_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Deaktiver læsning `~/.claude/CLAUDE.md`                               |
| `OPENLOOP_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Deaktiver indlæsning af `.claude/skills`                              |
| `OPENLOOP_DISABLE_MODELS_FETCH`       | boolean | Deaktivering af modeller fra eksterne kilder                          |
| `OPENLOOP_FAKE_VCS`                   | string  | Falsk VCS-udbyder til testformål                                      |
| `OPENLOOP_CLIENT`                     | string  | Klient-id (standard til `cli`)                                        |
| `OPENLOOP_ENABLE_EXA`                 | boolean | Aktiver Exa-websøgeværktøjer                                          |
| `OPENLOOP_SERVER_PASSWORD`            | string  | Aktiver grundlæggende godkendelse for `serve`/`web`                   |
| `OPENLOOP_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `opencode`) |
| `OPENLOOP_MODELS_URL`                 | string  | Brugerdefineret URL til hentning af modelkonfiguration                |
...
| Variabel                                        | Skriv   | Beskrivelse                                          |
| ----------------------------------------------- | ------- | ---------------------------------------------------- |
| `OPENLOOP_EXPERIMENTAL`                         | boolean | Aktiver eksperimentelle funktioner under samleflaget |
| `OPENLOOP_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Aktiver ikonopdagelse                                |
| `OPENLOOP_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Deaktiver kopi ved valg i TUI                        |
| `OPENLOOP_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | nummer  | Standard timeout for bash-kommandoer i ms            |
| `OPENLOOP_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | nummer  | Maks. output-tokens for LLM-svar                     |
| `OPENLOOP_EXPERIMENTAL_FILEWATCHER`             | boolean | Aktiver filovervågning for hele dir                  |
| `OPENLOOP_EXPERIMENTAL_OXFMT`                   | boolean | Aktiver oxfmt formatter                              |
| `OPENLOOP_EXPERIMENTAL_LSP_TOOL`                | boolean | Aktive eksperimenter LSP værktøj                     |
| `OPENLOOP_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Deaktiver filovervågning                             |
| `OPENLOOP_EXPERIMENTAL_EXA`                     | boolean | Aktive eksperimenter Exa-funktioner                  |
| `OPENLOOP_EXPERIMENTAL_LSP_TY`                  | boolean | Aktiver TY LSP for python-filer                      |
| `OPENLOOP_EXPERIMENTAL_PLAN_MODE`               | boolean | Aktiver plantilstand                                 |
| `OPENLOOP_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Aktiver baggrundsopgaver for subagenter              |
| `OPENLOOP_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Aktiver eksperimentelt hændelsessystem               |
| `OPENLOOP_EXPERIMENTAL_NATIVE_LLM`              | boolean | Aktiver native LLM-anmodningssti                     |
| `OPENLOOP_EXPERIMENTAL_PARALLEL`                | boolean | Aktiver parallel udførelse af websøgning             |
| `OPENLOOP_EXPERIMENTAL_SCOUT`                   | boolean | Aktiver Scout-subagent                               |
| `OPENLOOP_EXPERIMENTAL_WORKSPACES`              | boolean | Aktiver workspace-understøttelse                     |
