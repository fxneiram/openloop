...
:::note
This tool is only available when `OPENLOOP_EXPERIMENTAL_LSP_TOOL=true` (or `OPENLOOP_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the OpenCode provider or when the `OPENLOOP_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).
