...
- 项目配置：`.openloop/skills/<name>/SKILL.md`
- 全局配置：`~/.config/openloop/skills/<name>/SKILL.md`
...
在此过程中，它会加载 `.openloop/` 中所有匹配的 `skills/*/SKILL.md`，以及匹配的 `.claude/skills/*/SKILL.md` 或 `.agents/skills/*/SKILL.md`。

全局定义也会从 `~/.config/openloop/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md` 和 `~/.agents/skills/*/SKILL.md` 中加载。
...
创建 `.openloop/skills/git-release/SKILL.md`，内容如下：
...
在 `openloop.json` 中使用基于模式的权限来控制代理可以访问哪些技能：
