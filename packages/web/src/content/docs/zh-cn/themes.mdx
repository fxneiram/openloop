...
1. **内置主题** — 嵌入在二进制文件中
2. **用户配置目录** — 定义在 `~/.config/openloop/themes/*.json` 或 `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **项目根目录** — 定义在 `<project-root>/.openloop/themes/*.json`
4. **当前工作目录** — 定义在 `./.openloop/themes/*.json`
