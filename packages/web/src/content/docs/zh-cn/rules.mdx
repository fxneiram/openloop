...
您还可以在 `~/.config/openloop/AGENTS.md` 文件中设置全局规则。这些规则会应用于所有 opencode 会话。
...
- **全局规则**：`~/.claude/CLAUDE.md`（在没有 `~/.config/openloop/AGENTS.md` 的情况下使用）
...
2. **全局文件**，位于 `~/.config/openloop/AGENTS.md`
3. **Claude Code 文件**，位于 `~/.claude/CLAUDE.md`（除非已禁用）

在每个类别中，第一个匹配的文件优先。例如，如果您同时拥有 `AGENTS.md` 和 `CLAUDE.md`，则只会使用 `AGENTS.md`。同样，`~/.config/openloop/AGENTS.md` 优先于 `~/.claude/CLAUDE.md`。

---

## 自定义指令

您可以在 `openloop.json` 或全局配置文件 `~/.config/openloop/openloop.json` 中指定自定义指令文件。这允许您和团队复用现有规则，而无需将它们复制到 AGENTS.md 中。
...
推荐的方式是使用 `openloop.json` 中的 `instructions` 字段：
...
:::tip
对于 monorepo 或具有共享标准的项目，使用 `openloop.json` 配合 glob 模式（如 `packages/*/AGENTS.md`）比手动指定指令更易于维护。
:::
