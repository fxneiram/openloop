...
- **macOS/Linux**（旧版安装）: `~/.local/share/opencode/openloop.jsonc`
- **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.config\opencode\openloop.jsonc`
...
如果你的 `openloop.json(c)` 包含 `server` 部分，请临时移除该部分并重新启动桌面应用。

#### 检查环境变量

如果你在环境中设置了 `OPENLOOP_PORT`，桌面应用将尝试使用该端口作为本地服务器端口。

- 取消设置 `OPENLOOP_PORT`（或选择一个空闲端口）并重新启动。
