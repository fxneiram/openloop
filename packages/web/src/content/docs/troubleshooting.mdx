...
- **macOS/Linux**: `~/.config/openloop/openloop.jsonc` (or `~/.config/openloop/openloop.json`)
- **macOS/Linux** (older installs): `~/.local/share/opencode/openloop.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\opencode\openloop.jsonc`
...
If your `openloop.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.

#### Check environment variables

If you have `OPENLOOP_PORT` set in your environment, the desktop app will try to use that port for the local server.

- Unset `OPENLOOP_PORT` (or pick a free port) and restart.
