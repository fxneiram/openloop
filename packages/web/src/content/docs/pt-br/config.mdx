...
2. **Configuração global** (`~/.config/openloop/openloop.json`) - preferências do usuário
3. **Configuração personalizada** (`OPENLOOP_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`openloop.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.openloop`** - agentes, comandos, plugins
6. **Configuração inline** (`OPENLOOP_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.openloop` e `~/.config/openloop` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
Coloque sua configuração global do opencode em `~/.config/openloop/openloop.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/openloop/tui.json`.
...
Adicione `openloop.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `OPENLOOP_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `OPENLOOP_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.openloop`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.openloop`, então ele **pode substituir** suas configurações.
...
Use `OPENLOOP_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.

Chaves legadas `theme`, `keybinds` e `tui` em `openloop.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/openloop/agents/` ou `.openloop/agents/`. [Saiba mais aqui](/docs/agents).
...
Você também pode definir comandos usando arquivos markdown em `~/.config/openloop/commands/` ou `.openloop/commands/`. [Saiba mais aqui](/docs/commands).
...
Coloque arquivos de plugin em `.openloop/plugins/` ou `~/.config/openloop/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
