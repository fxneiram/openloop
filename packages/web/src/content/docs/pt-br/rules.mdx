...
Você também pode ter regras globais em um arquivo `~/.config/openloop/AGENTS.md`. Isso é aplicado em todas as sessões do opencode.
...
- **Regras globais**: `~/.claude/CLAUDE.md` (usado se não existir `~/.config/openloop/AGENTS.md`)
...
2. **Arquivo global** em `~/.config/openloop/AGENTS.md`
3. **Arquivo Claude Code** em `~/.claude/CLAUDE.md` (a menos que desabilitado)

O primeiro arquivo correspondente vence em cada categoria. Por exemplo, se você tiver tanto `AGENTS.md` quanto `CLAUDE.md`, apenas `AGENTS.md` é usado. Da mesma forma, `~/.config/openloop/AGENTS.md` tem precedência sobre `~/.claude/CLAUDE.md`.

---

## Instruções Personalizadas

Você pode especificar arquivos de instrução personalizados no seu `openloop.json` ou no global `~/.config/openloop/openloop.json`. Isso permite que você e sua equipe reutilizem regras existentes em vez de ter que duplicá-las no AGENTS.md.
...
### Usando openloop.json

A abordagem recomendada é usar o campo `instructions` em `openloop.json`:
...
:::tip
Para monorepos ou projetos com padrões compartilhados, usar `openloop.json` com padrões glob (como `packages/*/AGENTS.md`) é mais sustentável do que instruções manuais.
:::
