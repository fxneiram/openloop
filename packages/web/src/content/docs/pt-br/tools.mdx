...
:::note
Esta ferramenta está disponível apenas quando `OPENLOOP_EXPERIMENTAL_LSP_TOOL=true` (ou `OPENLOOP_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor opencode ou quando a variável de ambiente `OPENLOOP_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).
