...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `OPENLOOP_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENLOOP_SERVER_USERNAME` ou `opencode`) |
...
Isso inicia um servidor HTTP que fornece acesso à funcionalidade do opencode sem a interface TUI. Defina `OPENLOOP_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencode`).
...
Isso inicia um servidor HTTP e abre um navegador para acessar o opencode através de uma interface web. Defina `OPENLOOP_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencode`).
...
| ------------------------------------- | ------- | --------------------------------------------------------------------- |
| `OPENLOOP_AUTO_SHARE`                 | boolean | Compartilhar sessões automaticamente                                  |
| `OPENLOOP_GIT_BASH_PATH`              | string  | Caminho para o executável do Git Bash no Windows                      |
| `OPENLOOP_CONFIG`                     | string  | Caminho para o arquivo de configuração                                |
| `OPENLOOP_CONFIG_DIR`                 | string  | Caminho para o diretório de configuração                              |
| `OPENLOOP_CONFIG_CONTENT`             | string  | Conteúdo de configuração json inline                                  |
| `OPENLOOP_DISABLE_AUTOUPDATE`         | boolean | Desabilitar verificações de atualização automática                    |
| `OPENLOOP_DISABLE_PRUNE`              | boolean | Desabilitar a poda de dados antigos                                   |
| `OPENLOOP_DISABLE_TERMINAL_TITLE`     | boolean | Desabilitar atualizações automáticas do título do terminal            |
| `OPENLOOP_PERMISSION`                 | string  | Configuração de permissões json inline                                |
| `OPENLOOP_DISABLE_DEFAULT_PLUGINS`    | boolean | Desabilitar plugins padrão                                            |
| `OPENLOOP_DISABLE_LSP_DOWNLOAD`       | boolean | Desabilitar downloads automáticos do servidor LSP                     |
| `OPENLOOP_ENABLE_EXPERIMENTAL_MODELS` | boolean | Habilitar modelos experimentais                                       |
| `OPENLOOP_DISABLE_AUTOCOMPACT`        | boolean | Desabilitar compactação automática de contexto                        |
| `OPENLOOP_DISABLE_CLAUDE_CODE`        | boolean | Desabilitar leitura de `.claude` (prompt + habilidades)               |
| `OPENLOOP_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Desabilitar leitura de `~/.claude/CLAUDE.md`                          |
| `OPENLOOP_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Desabilitar carregamento de `.claude/skills`                          |
| `OPENLOOP_DISABLE_MODELS_FETCH`       | boolean | Desabilitar busca de modelos de fontes remotas                        |
| `OPENLOOP_FAKE_VCS`                   | string  | Provedor VCS falso para fins de teste                                 |
| `OPENLOOP_CLIENT`                     | string  | Identificador do cliente (padrão é `cli`)                             |
| `OPENLOOP_ENABLE_EXA`                 | boolean | Habilitar ferramentas de busca web Exa                                |
| `OPENLOOP_SERVER_PASSWORD`            | string  | Habilitar autenticação básica para `serve`/`web`                      |
| `OPENLOOP_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `opencode`) |
| `OPENLOOP_MODELS_URL`                 | string  | URL personalizada para buscar configuração de modelos                 |
...
| Variável                                        | Tipo    | Descrição                                                 |
| ----------------------------------------------- | ------- | --------------------------------------------------------- |
| `OPENLOOP_EXPERIMENTAL`                         | boolean | Habilitar recursos experimentais sob o sinalizador global |
| `OPENLOOP_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Habilitar descoberta de ícones                            |
| `OPENLOOP_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Desabilitar cópia ao selecionar no TUI                    |
| `OPENLOOP_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Tempo limite padrão para comandos bash em ms              |
| `OPENLOOP_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Máximo de tokens de saída para respostas LLM              |
| `OPENLOOP_EXPERIMENTAL_FILEWATCHER`             | boolean | Habilitar monitoramento de arquivos para todo o diretório |
| `OPENLOOP_EXPERIMENTAL_OXFMT`                   | boolean | Habilitar formatador oxfmt                                |
| `OPENLOOP_EXPERIMENTAL_LSP_TOOL`                | boolean | Habilitar ferramenta LSP experimental                     |
| `OPENLOOP_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Desabilitar monitoramento de arquivos                     |
| `OPENLOOP_EXPERIMENTAL_EXA`                     | boolean | Habilitar recursos experimentais do Exa                   |
| `OPENLOOP_EXPERIMENTAL_LSP_TY`                  | boolean | Habilitar TY LSP para arquivos python                     |
| `OPENLOOP_EXPERIMENTAL_PLAN_MODE`               | boolean | Habilitar modo de plano                                   |
| `OPENLOOP_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Habilitar tarefas de subagentes em segundo plano          |
| `OPENLOOP_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Habilitar sistema de eventos experimental                 |
| `OPENLOOP_EXPERIMENTAL_NATIVE_LLM`              | boolean | Habilitar caminho nativo de requisição LLM                |
| `OPENLOOP_EXPERIMENTAL_PARALLEL`                | boolean | Habilitar execução paralela de busca web                  |
| `OPENLOOP_EXPERIMENTAL_SCOUT`                   | boolean | Habilitar subagente Scout                                 |
| `OPENLOOP_EXPERIMENTAL_WORKSPACES`              | boolean | Habilitar suporte a espaços de trabalho                   |
