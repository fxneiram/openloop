...
- **macOS/Linux** (instalações mais antigas): `~/.local/share/opencode/openloop.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\opencode\openloop.jsonc`
...
Se seu `openloop.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `OPENLOOP_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `OPENLOOP_PORT` (ou escolha uma porta livre) e reinicie.
