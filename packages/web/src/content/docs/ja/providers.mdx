...
2. **次のいずれかの方法を使用して認証を構成します**:
   ...
   ***
   ...
   プロジェクト固有の設定または永続的な設定の場合は、`openloop.json` を使用します。
...
`openloop.json` ファイル。セッション共有を無効にすることもお勧めします。
...
4. プロジェクトディレクトリで `openloop.json` ファイルを作成または更新します。
