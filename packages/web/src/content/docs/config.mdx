...
2. **Global config** (`~/.config/openloop/openloop.json`) - user preferences
3. **Custom config** (`OPENLOOP_CONFIG` env var) - custom overrides
4. **Project config** (`openloop.json` in project) - project-specific settings
5. **`.openloop` directories** - agents, commands, plugins
6. **Inline config** (`OPENLOOP_CONFIG_CONTENT` env var) - runtime overrides
...
:::note
The `.openloop` and `~/.config/openloop` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Place your global OpenCode config in `~/.config/openloop/openloop.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/openloop/tui.json`.
...
Add `openloop.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
Specify a custom config file path using the `OPENLOOP_CONFIG` environment variable.
...
Specify a custom config directory using the `OPENLOOP_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.openloop` directory, and should
...
The custom directory is loaded after the global config and `.openloop` directories, so it **can override** their settings.
...
Drop an `openloop.json` or `openloop.jsonc` file in the system managed config directory:
...
The plist keys map directly to `openloop.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.
...
Use `OPENLOOP_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `openloop.json` are deprecated and automatically migrated when possible.
...
You can also define agents using markdown files in `~/.config/openloop/agents/` or `.openloop/agents/`. [Learn more here](/docs/agents).
...
You can also define commands using markdown files in `~/.config/openloop/commands/` or `.openloop/commands/`. [Learn more here](/docs/commands).
...
Place plugin files in `.openloop/plugins/` or `~/.config/openloop/plugins/`. You can also load plugins from npm through the `plugin` option.
