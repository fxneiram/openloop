...
:::caution
If `OPENLOOP_SERVER_PASSWORD` is not set, the server will be unsecured. This is fine for local use but should be set for network access.
:::
...
To protect access, set a password using the `OPENLOOP_SERVER_PASSWORD` environment variable:

```bash
OPENLOOP_SERVER_PASSWORD=secret opencode web
```

The username defaults to `opencode` but can be changed with `OPENLOOP_SERVER_USERNAME`.
...
You can also configure server settings in your `openloop.json` config file:
