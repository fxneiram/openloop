...
:::note
Dette verktøyet er bare tilgjengelig når `OPENLOOP_EXPERIMENTAL_LSP_TOOL=true` (eller `OPENLOOP_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgjengelig når du bruker OpenCode-leverandøren eller når miljøvariabelen `OPENLOOP_ENABLE_EXA` er satt til en sann verdi (f.eks. `true` eller `1`).
