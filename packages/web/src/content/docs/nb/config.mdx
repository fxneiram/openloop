...
2. **Global konfigurasjon** (`~/.config/openloop/openloop.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`OPENLOOP_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`openloop.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.openloop` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`OPENLOOP_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.openloop` og `~/.config/openloop` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Plasser din globale OpenCode-konfigurasjon i `~/.config/openloop/openloop.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `openloop.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `OPENLOOP_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `OPENLOOP_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.openloop` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.openloop`-katalogene, slik at den **kan overstyre** innstillingene deres.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/openloop/agents/` eller `.openloop/agents/`. [Les mer her](/docs/agents).
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/openloop/commands/` eller `.openloop/commands/`. [Les mer her](/docs/commands).
...
Plasser plugin-filer i `.openloop/plugins/` eller `~/.config/openloop/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
