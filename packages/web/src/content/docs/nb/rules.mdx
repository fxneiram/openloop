...
Du kan også ha globale regler i en `~/.config/openloop/AGENTS.md`-fil. Dette blir brukt på alle OpenCode-økter.
...
- **Globale regler**: `~/.claude/CLAUDE.md` (brukes hvis ingen `~/.config/openloop/AGENTS.md` eksisterer)
...
Den første matchende filen vinner i hver kategori. For eksempel, hvis du har både `AGENTS.md` og `CLAUDE.md`, brukes bare `AGENTS.md`. På samme måte har `~/.config/openloop/AGENTS.md` forrang over `~/.claude/CLAUDE.md`.

---

## Egendefinerte instruksjoner

Du kan spesifisere egendefinerte instruksjonsfiler i din `openloop.json` eller den globale `~/.config/openloop/openloop.json`. Dette lar deg og teamet ditt gjenbruke eksisterende regler i stedet for å måtte duplisere dem til AGENTS.md.
...
### Bruker openloop.json

Den anbefalte tilnærmingen er å bruke feltet `instructions` i `openloop.json`:
...
:::tip
For monorepos eller prosjekter med delte standarder er bruk av `openloop.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::
