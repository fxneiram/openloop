...
For å beskytte tilgang, angi et passord ved hjelp av miljøvariabelen `OPENLOOP_SERVER_PASSWORD`:

```bash
OPENLOOP_SERVER_PASSWORD=secret opencode web
```

Brukernavnet er satt til `opencode` som standard, men kan endres med `OPENLOOP_SERVER_USERNAME`.
...
Du kan også konfigurere serverinnstillinger i `openloop.json` konfigurasjonsfilen:
