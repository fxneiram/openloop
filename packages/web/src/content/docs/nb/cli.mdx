...
| <nobr><code>{"--session"}</code></nobr>  | `-s` | Økt ID for å fortsette                                                                             |
| <nobr><code>{"--fork"}</code></nobr>     |      | Forgren økten ved fortsettelse (bruk med `--continue` eller `--session`)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Passord for grunnleggende autentisering (standard: `OPENLOOP_SERVER_PASSWORD`)                     |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENLOOP_SERVER_USERNAME` eller `opencode`) |
...
Dette starter en HTTP-server som gir API tilgang til OpenCode-funksjonalitet uten TUI-grensesnittet. Sett `OPENLOOP_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `opencode`).
...
Dette starter en HTTP-server og åpner en nettleser for å få tilgang til OpenCode via et nettgrensesnitt. Sett `OPENLOOP_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `opencode`).
...
| ------------------------------------- | ------ | --------------------------------------------------------------------- |
| `OPENLOOP_AUTO_SHARE`                 | boolsk | Del økter automatisk                                                  |
| `OPENLOOP_GIT_BASH_PATH`              | streng | Bane til Git Bash-kjørbar på Windows                                  |
| `OPENLOOP_CONFIG`                     | streng | Bane til konfigurasjonsfil                                            |
| `OPENLOOP_TUI_CONFIG`                 | streng | Bane til TUI-konfigurasjonsfil                                        |
| `OPENLOOP_CONFIG_DIR`                 | streng | Bane til konfigurasjonskatalog                                        |
| `OPENLOOP_CONFIG_CONTENT`             | streng | Innebygd json-konfigurasjonsinnhold                                   |
| `OPENLOOP_DISABLE_AUTOUPDATE`         | boolsk | Deaktiver automatiske oppdateringskontroller                          |
| `OPENLOOP_DISABLE_PRUNE`              | boolsk | Deaktiver beskjæring av gamle data                                    |
| `OPENLOOP_DISABLE_TERMINAL_TITLE`     | boolsk | Deaktiver automatiske terminaltitteloppdateringer                     |
| `OPENLOOP_PERMISSION`                 | streng | Innebygd json-tillatelseskonfigurasjon                                |
| `OPENLOOP_DISABLE_DEFAULT_PLUGINS`    | boolsk | Deaktiver standard plugins                                            |
| `OPENLOOP_DISABLE_LSP_DOWNLOAD`       | boolsk | Deaktiver automatiske LSP servernedlastinger                          |
| `OPENLOOP_ENABLE_EXPERIMENTAL_MODELS` | boolsk | Aktiver eksperimentelle modeller                                      |
| `OPENLOOP_DISABLE_AUTOCOMPACT`        | boolsk | Deaktiver automatisk kontekstkomprimering                             |
| `OPENLOOP_DISABLE_CLAUDE_CODE`        | boolsk | Deaktiver lesing fra `.claude` (spørsmål + ferdigheter)               |
| `OPENLOOP_DISABLE_CLAUDE_CODE_PROMPT` | boolsk | Deaktiver lesing `~/.claude/CLAUDE.md`                                |
| `OPENLOOP_DISABLE_CLAUDE_CODE_SKILLS` | boolsk | Deaktiver innlasting av `.claude/skills`                              |
| `OPENLOOP_DISABLE_MODELS_FETCH`       | boolsk | Deaktiver henting av modeller fra eksterne kilder                     |
| `OPENLOOP_FAKE_VCS`                   | streng | Falsk VCS-leverandør for testformål                                   |
| `OPENLOOP_CLIENT`                     | streng | Klientidentifikator (standard til `cli`)                              |
| `OPENLOOP_ENABLE_EXA`                 | boolsk | Aktiver Exa-nettsøkeverktøy                                           |
| `OPENLOOP_SERVER_PASSWORD`            | streng | Aktiver grunnleggende autentisering for `serve`/`web`                 |
| `OPENLOOP_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `opencode`) |
| `OPENLOOP_MODELS_URL`                 | streng | Egendefinert URL for henting av modellkonfigurasjon                   |
...
| Variabel                                        | Type   | Beskrivelse                                           |
| ----------------------------------------------- | ------ | ----------------------------------------------------- |
| `OPENLOOP_EXPERIMENTAL`                         | boolsk | Aktiver eksperimentelle funksjoner under samleflagget |
| `OPENLOOP_EXPERIMENTAL_ICON_DISCOVERY`          | boolsk | Aktiver ikonoppdagelse                                |
| `OPENLOOP_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolsk | Deaktiver kopi ved valg i TUI                         |
| `OPENLOOP_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | tall   | Standard tidsavbrudd for bash-kommandoer i ms         |
| `OPENLOOP_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | tall   | Maks Output Tokens for LLM-svar                       |
| `OPENLOOP_EXPERIMENTAL_FILEWATCHER`             | boolsk | Aktiver filovervåker for hele dir                     |
| `OPENLOOP_EXPERIMENTAL_OXFMT`                   | boolsk | Aktiver oxfmt formatter                               |
| `OPENLOOP_EXPERIMENTAL_LSP_TOOL`                | boolsk | Aktiver eksperimentelt LSP-verktøy                    |
| `OPENLOOP_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolsk | Deaktiver filovervåking                               |
| `OPENLOOP_EXPERIMENTAL_EXA`                     | boolsk | Aktiver eksperimentelle Exa-funksjoner                |
| `OPENLOOP_EXPERIMENTAL_LSP_TY`                  | boolsk | Aktiver TY LSP for python-filer                       |
| `OPENLOOP_EXPERIMENTAL_PLAN_MODE`               | boolsk | Aktiver planmodus                                     |
| `OPENLOOP_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolsk | Aktiver bakgrunnsoppgaver for subagenter              |
| `OPENLOOP_EXPERIMENTAL_EVENT_SYSTEM`            | boolsk | Aktiver eksperimentelt hendelsessystem                |
| `OPENLOOP_EXPERIMENTAL_NATIVE_LLM`              | boolsk | Aktiver innebygd LLM-forespørselsvei                  |
| `OPENLOOP_EXPERIMENTAL_PARALLEL`                | boolsk | Aktiver parallell kjøring av websøk                   |
| `OPENLOOP_EXPERIMENTAL_SCOUT`                   | boolsk | Aktiver Scout-subagent                                |
| `OPENLOOP_EXPERIMENTAL_WORKSPACES`              | boolsk | Aktiver arbeidsområde-støtte                          |
