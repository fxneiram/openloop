...
- **macOS/Linux**: `~/.config/openloop/openloop.jsonc` (eller `~/.config/openloop/openloop.json`)
- **macOS/Linux** (eldre installasjoner): `~/.local/share/opencode/openloop.jsonc`
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\opencode\openloop.jsonc`
...
Hvis `openloop.json(c)` inneholder en `server`-del, fjern den midlertidig og start skrivebordsappen på nytt.

#### Sjekk miljøvariabler

Hvis du har `OPENLOOP_PORT` satt i miljøet ditt, vil skrivebordsappen prøve å bruke den porten for den lokale serveren.

- Deaktiver `OPENLOOP_PORT` (eller velg en ledig port) og start på nytt.
