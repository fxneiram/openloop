...
You can also have global rules in a `~/.config/openloop/AGENTS.md` file. This gets applied across all opencode sessions.
...
- **Global rules**: `~/.claude/CLAUDE.md` (used if no `~/.config/openloop/AGENTS.md` exists)
...
2. **Global file** at `~/.config/openloop/AGENTS.md`
3. **Claude Code file** at `~/.claude/CLAUDE.md` (unless disabled)

The first matching file wins in each category. For example, if you have both `AGENTS.md` and `CLAUDE.md`, only `AGENTS.md` is used. Similarly, `~/.config/openloop/AGENTS.md` takes precedence over `~/.claude/CLAUDE.md`.

---

## Custom Instructions

You can specify custom instruction files in your `openloop.json` or the global `~/.config/openloop/openloop.json`. This allows you and your team to reuse existing rules rather than having to duplicate them to AGENTS.md.
...
The recommended approach is to use the `instructions` field in `openloop.json`:
...
:::tip
For monorepos or projects with shared standards, using `openloop.json` with glob patterns (like `packages/*/AGENTS.md`) is more maintainable than manual instructions.
:::
