...
您還可以在 `~/.config/openloop/AGENTS.md` 檔案中設定全域規則。這些規則會套用於所有 opencode 工作階段。
...
- **全域規則**：`~/.claude/CLAUDE.md`（在沒有 `~/.config/openloop/AGENTS.md` 的情況下使用）
...
2. **全域檔案**，位於 `~/.config/openloop/AGENTS.md`
3. **Claude Code 檔案**，位於 `~/.claude/CLAUDE.md`（除非已停用）

在每個類別中，第一個符合的檔案優先。例如，如果您同時擁有 `AGENTS.md` 和 `CLAUDE.md`，則只會使用 `AGENTS.md`。同樣，`~/.config/openloop/AGENTS.md` 優先於 `~/.claude/CLAUDE.md`。

---

## 自訂指令

您可以在 `openloop.json` 或全域設定檔 `~/.config/openloop/openloop.json` 中指定自訂指令檔案。這允許您和團隊複用現有規則，而無需將它們複製到 AGENTS.md 中。
...
建議的方式是使用 `openloop.json` 中的 `instructions` 欄位：
...
:::tip
對於 monorepo 或具有共享標準的專案，使用 `openloop.json` 搭配 glob 模式（如 `packages/*/AGENTS.md`）比手動指定指令更易於維護。
:::
