...
1. **內建主題** — 嵌入在二進位檔案中
2. **使用者設定目錄** — 定義在 `~/.config/openloop/themes/*.json` 或 `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **專案根目錄** — 定義在 `<project-root>/.openloop/themes/*.json`
4. **當前工作目錄** — 定義在 `./.openloop/themes/*.json`
