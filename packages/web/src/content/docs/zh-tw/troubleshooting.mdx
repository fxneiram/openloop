...
- **macOS/Linux**: `~/.config/openloop/openloop.jsonc`（或 `~/.config/openloop/openloop.json`）
- **macOS/Linux**（舊版安裝）: `~/.local/share/opencode/openloop.jsonc`
- **Windows**: 按 `WIN+R` 並貼上 `%USERPROFILE%\.config\opencode\openloop.jsonc`
...
如果你的 `openloop.json(c)` 包含 `server` 部分，請暫時移除該部分並重新啟動桌面應用程式。

#### 檢查環境變數

如果你在環境中設定了 `OPENLOOP_PORT`，桌面應用程式將嘗試使用該連接埠作為本地伺服器連接埠。

- 取消設定 `OPENLOOP_PORT`（或選擇一個空閒連接埠）並重新啟動。
