...
- Конфигурация проекта: `.openloop/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/openloop/skills/<name>/SKILL.md`.
...
Он загружает все соответствующие `skills/*/SKILL.md` в `.openloop/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/openloop/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.openloop/skills/git-release/SKILL.md` следующим образом:
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `openloop.json`:
...
**Для встроенных агентов** (в формате `openloop.json`):
