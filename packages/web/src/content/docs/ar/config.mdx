...
2. **الإعدادات العامة** (`~/.config/openloop/openloop.json`) - تفضيلات المستخدم
3. **إعدادات مخصصة** (`OPENLOOP_CONFIG` env var) - تجاوزات مخصصة
4. **إعدادات المشروع** (`openloop.json` داخل المشروع) - إعدادات خاصة بالمشروع
5. **أدلة `.openloop`** - الوكلاء، الأوامر، الإضافات
6. **إعدادات ضمنية** (`OPENLOOP_CONFIG_CONTENT` env var) - تجاوزات وقت التشغيل
...
:::note
يستخدم الدليلان `.openloop` و`~/.config/openloop` **أسماء جمع** للأدلة الفرعية: `agents/` و`commands/` و`modes/` و`plugins/` و`skills/` و`tools/` و`themes/`. كما تُدعَم أسماء المفرد (مثل `agent/`) للتوافق مع الإصدارات السابقة.
:::
...
ضع إعدادات OpenCode العامة في `~/.config/openloop/openloop.json`. استخدم الإعدادات العامة للتفضيلات على مستوى المستخدم مثل السمات، والمزوّدين، أو اختصارات المفاتيح.
...
أضف `openloop.json` في جذر مشروعك. تمتلك إعدادات المشروع أعلى أولوية بين ملفات الإعدادات القياسية، إذ تتجاوز الإعدادات العامة والبعيدة معًا.
...
حدّد مسار ملف إعدادات مخصصًا باستخدام متغير البيئة `OPENLOOP_CONFIG`.
...
حدّد دليل إعدادات مخصصًا باستخدام متغير البيئة `OPENLOOP_CONFIG_DIR`.
سيتم البحث داخل هذا الدليل عن الوكلاء والأوامر والأوضاع والإضافات تمامًا مثل
الدليل القياسي `.openloop`، ويجب أن يتبع نفس البنية.
...
يُحمَّل الدليل المخصص بعد الإعدادات العامة وأدلة `.openloop`، لذلك **يمكنه تجاوز** إعداداتها.
...
يمكنك أيضًا تعريف الوكلاء باستخدام ملفات markdown في `~/.config/openloop/agents/` أو `.openloop/agents/`. [تعرف على المزيد هنا](/docs/agents).
...
يمكنك أيضًا تعريف الأوامر باستخدام ملفات markdown في `~/.config/openloop/commands/` أو `.openloop/commands/`. [تعرف على المزيد هنا](/docs/commands).
...
ضع ملفات الإضافات في `.openloop/plugins/` أو `~/.config/openloop/plugins/`. يمكنك أيضًا تحميل إضافات من npm عبر الخيار `plugin`.
