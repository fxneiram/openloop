...
يمكنك أيضًا وضع قواعد عامة في ملف `~/.config/openloop/AGENTS.md`. تُطبَّق هذه القواعد على جميع جلسات opencode.
...
- **قواعد عامة**: `~/.claude/CLAUDE.md` (يُستخدم إذا لم يوجد `~/.config/openloop/AGENTS.md`)
...
2. **ملف عام** في `~/.config/openloop/AGENTS.md`
3. **ملف Claude Code** في `~/.claude/CLAUDE.md` (ما لم يتم تعطيله)

يتم اعتماد أول ملف مطابق ضمن كل فئة. على سبيل المثال، إذا كان لديك كل من `AGENTS.md` و`CLAUDE.md` فسيتم استخدام `AGENTS.md` فقط. وبالمثل، تكون الأولوية لملف `~/.config/openloop/AGENTS.md` على `~/.claude/CLAUDE.md`.

---

## تعليمات مخصصة

يمكنك تحديد ملفات تعليمات مخصصة في `openloop.json` أو في الملف العام `~/.config/openloop/openloop.json`. يتيح ذلك لك ولفريقك إعادة استخدام القواعد الموجودة بدلًا من تكرارها داخل AGENTS.md.
...
### استخدام openloop.json

النهج الموصى به هو استخدام الحقل `instructions` في `openloop.json`:
...
:::tip
بالنسبة للمستودعات متعددة الحزم (monorepos) أو المشاريع ذات المعايير المشتركة، فإن استخدام `openloop.json` مع أنماط glob (مثل `packages/*/AGENTS.md`) أسهل للصيانة من التعليمات اليدوية.
:::
