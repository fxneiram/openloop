...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/openloop/themes/*.json` أو `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.openloop/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.openloop/themes/*.json`
