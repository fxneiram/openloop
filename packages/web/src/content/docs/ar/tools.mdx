...
:::note
هذه الأداة متاحة فقط عند ضبط `OPENLOOP_EXPERIMENTAL_LSP_TOOL=true` (أو `OPENLOOP_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود OpenCode أو عند ضبط متغير البيئة `OPENLOOP_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).
