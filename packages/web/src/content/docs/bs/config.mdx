...
2. **Globalna konfiguracija** (`~/.config/openloop/openloop.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`OPENLOOP_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`openloop.json` u projektu) - postavke specifične za projekat
5. **`.openloop` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`OPENLOOP_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.openloop` i `~/.config/openloop` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Postavite svoju globalnu OpenCode konfiguraciju u `~/.config/openloop/openloop.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/openloop/tui.json`.
...
Dodajte `openloop.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `OPENLOOP_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `OPENLOOP_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.openloop` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.openloop`, tako da **može nadjačati** njihove postavke.
...
Koristite `OPENLOOP_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `openloop.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/openloop/agents/` ili `.openloop/agents/`. [Saznajte više ovdje](/docs/agents).
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/openloop/commands/` ili `.openloop/commands/`. [Saznajte više ovdje](/docs/commands).
...
Postavite datoteke dodataka u `.openloop/plugins/` ili `~/.config/openloop/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
