...
:::caution
Ako `OPENLOOP_SERVER_PASSWORD` nije postavljen, server će biti nezaštićen. Ovo je u redu za lokalnu upotrebu, ali bi trebalo biti postavljeno za pristup mreži.
:::
...
Da biste zaštitili pristup, postavite lozinku koristeći varijablu okruženja `OPENLOOP_SERVER_PASSWORD`:

```bash
OPENLOOP_SERVER_PASSWORD=secret opencode web
```

Korisničko ime podrazumevano je `opencode`, ali se može promeniti sa `OPENLOOP_SERVER_USERNAME`.
...
Također možete konfigurirati postavke servera u svom `openloop.json` konfiguracijskom fajlu:
