...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `openloop.json`:
...
`openloop.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `openloop.json`:
...
4. Kreirajte ili ažurirajte svoju `openloop.json` datoteku u direktoriju projekta:
