...
| <nobr><code>{"--password"}</code></nobr> | `-p`   | Lozinka za osnovnu autentifikaciju (zadano: `OPENLOOP_SERVER_PASSWORD`)                       |
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENLOOP_SERVER_USERNAME` ili `opencode`) |
...
Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti OpenCode-a bez TUI interfejsa. Postavite `OPENLOOP_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Ovo pokreće HTTP server i otvara web pretraživač za pristup OpenCode-u preko web interfejsa. Postavite `OPENLOOP_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
| Varijabla                             | Tip     | Opis                                                               |
| ------------------------------------- | ------- | ------------------------------------------------------------------ |
| `OPENLOOP_AUTO_SHARE`                 | boolean | Automatski dijeli sesije                                           |
| `OPENLOOP_GIT_BASH_PATH`              | string  | Putanja do Git Bash izvršne datoteke na Windows-u                  |
| `OPENLOOP_CONFIG`                     | string  | Putanja do konfiguracijskog fajla                                  |
| `OPENLOOP_TUI_CONFIG`                 | string  | Putanja do TUI konfiguracijskog fajla                              |
| `OPENLOOP_CONFIG_DIR`                 | string  | Putanja do konfiguracijskog direktorija                            |
| `OPENLOOP_CONFIG_CONTENT`             | string  | Inline json konfiguracijski sadržaj                                |
| `OPENLOOP_DISABLE_AUTOUPDATE`         | boolean | Onemogući automatske provjere ažuriranja                           |
| `OPENLOOP_DISABLE_PRUNE`              | boolean | Onemogući brisanje (pruning) starih podataka                       |
| `OPENLOOP_DISABLE_TERMINAL_TITLE`     | boolean | Onemogući automatsko ažuriranje naslova terminala                  |
| `OPENLOOP_PERMISSION`                 | string  | Inline json konfiguracija dozvola                                  |
| `OPENLOOP_DISABLE_DEFAULT_PLUGINS`    | boolean | Onemogući podrazumijevane dodatke (plugins)                        |
| `OPENLOOP_DISABLE_LSP_DOWNLOAD`       | boolean | Onemogući automatsko preuzimanje LSP servera                       |
| `OPENLOOP_ENABLE_EXPERIMENTAL_MODELS` | boolean | Omogući eksperimentalne modele                                     |
| `OPENLOOP_DISABLE_AUTOCOMPACT`        | boolean | Onemogući automatsko sažimanje konteksta                           |
| `OPENLOOP_DISABLE_CLAUDE_CODE`        | boolean | Onemogući čitanje iz `.claude` (prompt + vještine)                 |
| `OPENLOOP_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Onemogući čitanje `~/.claude/CLAUDE.md`                            |
| `OPENLOOP_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Onemogući učitavanje `.claude/skills`                              |
| `OPENLOOP_DISABLE_MODELS_FETCH`       | boolean | Onemogući dohvaćanje modela iz udaljenih izvora                    |
| `OPENLOOP_FAKE_VCS`                   | string  | Lažni VCS provajder za potrebe testiranja                          |
| `OPENLOOP_CLIENT`                     | string  | Identifikator klijenta (zadano na `cli`)                           |
| `OPENLOOP_ENABLE_EXA`                 | boolean | Omogući Exa alate za web pretraživanje                             |
| `OPENLOOP_SERVER_PASSWORD`            | string  | Omogući osnovnu autentifikaciju za `serve`/`web`                   |
| `OPENLOOP_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `opencode`) |
| `OPENLOOP_MODELS_URL`                 | string  | Prilagođeni URL za dohvaćanje konfiguracije modela                 |
...
| Varijabla                                       | Tip     | Opis                                                    |
| ----------------------------------------------- | ------- | ------------------------------------------------------- |
| `OPENLOOP_EXPERIMENTAL`                         | boolean | Omogući eksperimentalne funkcije pod zbirnom zastavicom |
| `OPENLOOP_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Omogući otkrivanje ikona                                |
| `OPENLOOP_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Onemogući kopiranje pri odabiru u TUI                   |
| `OPENLOOP_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Zadano vremensko ograničenje za bash naredbe u ms       |
| `OPENLOOP_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Maksimalni izlazni tokeni za LLM odgovore               |
| `OPENLOOP_EXPERIMENTAL_FILEWATCHER`             | boolean | Omogući praćenje datoteka za cijeli direktorij          |
| `OPENLOOP_EXPERIMENTAL_OXFMT`                   | boolean | Omogući oxfmt formatter                                 |
| `OPENLOOP_EXPERIMENTAL_LSP_TOOL`                | boolean | Omogući eksperimentalni LSP alat                        |
| `OPENLOOP_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Onemogući praćenje datoteka                             |
| `OPENLOOP_EXPERIMENTAL_EXA`                     | boolean | Omogući eksperimentalne Exa funkcije                    |
| `OPENLOOP_EXPERIMENTAL_LSP_TY`                  | boolean | Omogući TY LSP za python datoteke                       |
| `OPENLOOP_EXPERIMENTAL_PLAN_MODE`               | boolean | Omogući Plan mod                                        |
| `OPENLOOP_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Omogući pozadinske zadatke subagenata                   |
| `OPENLOOP_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Omogući eksperimentalni sistem događaja                 |
| `OPENLOOP_EXPERIMENTAL_NATIVE_LLM`              | boolean | Omogući nativnu putanju LLM zahtjeva                    |
| `OPENLOOP_EXPERIMENTAL_PARALLEL`                | boolean | Omogući paralelno izvršavanje web pretrage              |
| `OPENLOOP_EXPERIMENTAL_SCOUT`                   | boolean | Omogući Scout subagenta                                 |
| `OPENLOOP_EXPERIMENTAL_WORKSPACES`              | boolean | Omogući podršku za radne prostore                       |
