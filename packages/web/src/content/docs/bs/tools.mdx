...
:::note
Ovaj alat je dostupan samo kada je `OPENLOOP_EXPERIMENTAL_LSP_TOOL=true` (ili `OPENLOOP_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OpenCode provajdera ili kada je varijabla `OPENLOOP_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).
