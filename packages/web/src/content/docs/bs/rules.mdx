...
Mozete imati i globalna pravila u `~/.config/openloop/AGENTS.md`. Ona se primjenjuju u svim opencode sesijama.
...
- **Globalna pravila**: `~/.claude/CLAUDE.md` (koristi se ako ne postoji `~/.config/openloop/AGENTS.md`)
...
2. **Globalna datoteka** na `~/.config/openloop/AGENTS.md`
3. **Claude Code datoteka** na `~/.claude/CLAUDE.md` (osim ako je iskljucena)

Prva pronadena datoteka pobjeduje u svakoj kategoriji. Na primjer, ako imate i `AGENTS.md` i `CLAUDE.md`, koristi se samo `AGENTS.md`. Isto tako, `~/.config/openloop/AGENTS.md` ima prednost nad `~/.claude/CLAUDE.md`.

---

## Prilagođena uputstva

Mozete navesti prilagodene datoteke uputstava u `openloop.json` ili globalnom `~/.config/openloop/openloop.json`. Tako vi i tim ponovo koristite postojeca pravila bez dupliranja u AGENTS.md.
...
### Korištenje openloop.json

Preporuceni pristup je da koristite `instructions` polje u `openloop.json`:
...
:::tip
Za monorepo projekte ili projekte sa zajednickim standardima, odrzivije je koristiti `openloop.json` sa glob obrascima (npr. `packages/*/AGENTS.md`) nego rucna uputstva.
:::
