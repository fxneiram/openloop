...
- **macOS/Linux**: `~/.config/openloop/openloop.jsonc` (ili `~/.config/openloop/openloop.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/opencode/openloop.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\opencode\openloop.jsonc`
...
Ako vaš `openloop.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `OPENLOOP_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `OPENLOOP_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
