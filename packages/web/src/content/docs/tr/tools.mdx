...
:::note
Bu araç yalnızca `OPENLOOP_EXPERIMENTAL_LSP_TOOL=true` (veya `OPENLOOP_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca opencode provider kullanılırken veya `OPENLOOP_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.
