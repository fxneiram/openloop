...
:::caution
`OPENLOOP_SERVER_PASSWORD` ayarlanmadıysa sunucunun güvenliği kaldırılacaktır. Bu, yerel kullanım için iyidir ancak ağ erişimi için ayarlanmalıdır.
:::
...
Erişimi korumak için `OPENLOOP_SERVER_PASSWORD` ortam değişkenini kullanarak bir parola ayarlayın:

```bash
OPENLOOP_SERVER_PASSWORD=secret opencode web
```

Kullanıcı adı varsayılan olarak `opencode` şeklindedir ancak `OPENLOOP_SERVER_USERNAME` ile değiştirilebilir.
...
Sunucu ayarlarını `openloop.json` yapılandırma dosyanızda da yapılandırabilirsiniz:
