...
- **macOS/Linux**: `~/.config/openloop/openloop.jsonc` (veya `~/.config/openloop/openloop.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/opencode/openloop.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\opencode\openloop.jsonc` yapistirin
...
`openloop.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `OPENLOOP_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `OPENLOOP_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
