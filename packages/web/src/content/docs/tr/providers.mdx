...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   Projeye özel veya kalıcı yapılandırma için `openloop.json` kullanın:
...
`openloop.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`openloop.json` üzerinden özelleştirin:
...
4. Proje dizininizde `openloop.json` dosyanızı oluşturun veya güncelleyin:
