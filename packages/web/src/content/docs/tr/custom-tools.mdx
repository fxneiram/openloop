...
- Yerel olarak bunları projenizin `.openloop/tools/` dizinine yerleştirerek.
- Veya küresel olarak bunları `~/.config/openloop/tools/` içine yerleştirerek.
...
```ts title=".openloop/tools/database.ts" {1}
...
```ts title=".openloop/tools/math.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".openloop/tools/bash.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".openloop/tools/project.ts" {8}
import { tool } from "@opencode-ai/plugin"
...
```python title=".openloop/tools/add.py"
import sys
...
```ts title=".openloop/tools/python-add.ts" {10}
import { tool } from "@opencode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".openloop/tools/add.py")
