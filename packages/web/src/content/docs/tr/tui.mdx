...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `openloop.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `OPENLOOP_TUI_CONFIG` kullanın.
