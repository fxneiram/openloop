...
2. **Genel yapılandırma** (`~/.config/openloop/openloop.json`) - kullanıcı tercihleri
3. **Özel yapılandırma** (`OPENLOOP_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `openloop.json`) - projeye özgü ayarlar
5. **`.openloop` dizinleri** - agent'lar, komutlar, eklentiler
6. **Satır içi yapılandırma** (`OPENLOOP_CONFIG_CONTENT` env var) - çalışma zamanı geçersiz kılmaları
...
:::note
`.openloop` ve `~/.config/openloop` dizinleri alt dizinler için **çoğul adlar** kullanır: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` ve `themes/`. Tekil adlar (örneğin, `agent/`) da geriye dönük uyumluluk açısından desteklenir.
:::
...
Global opencode yapılandırmanızı `~/.config/openloop/openloop.json` içine yerleştirin. Temalar, sağlayıcılar veya tuş atamaları gibi kullanıcı çapındaki tercihler için genel yapılandırmayı kullanın.
...
Proje kökünüze `openloop.json` ekleyin. Proje yapılandırması, standart yapılandırma dosyaları arasında en yüksek önceliğe sahiptir; hem genel hem de uzak yapılandırmaları geçersiz kılar.
...
`OPENLOOP_CONFIG` ortam değişkenini kullanarak özel bir yapılandırma dosyası yolu belirtin.
...
`OPENLOOP_CONFIG_DIR` kullanarak özel bir yapılandırma dizini belirtin
ortam değişkeni. Bu dizin agent'lar, komutlar için aranacaktır.
modlar ve eklentiler tıpkı standart `.openloop` dizini gibi olmalıdır ve
...
Özel dizin, genel yapılandırma ve `.openloop` dizinlerinden sonra yüklenir, böylece bunların ayarlarını **geçersiz kılabilir**.
...
Agent'ları `~/.config/openloop/agents/` veya `.openloop/agents/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/agents).
...
Komutları `~/.config/openloop/commands/` veya `.openloop/commands/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/commands).
...
Eklenti dosyalarını `.openloop/plugins/` veya `~/.config/openloop/plugins/` içine yerleştirin. Ayrıca eklentileri `plugin` seçeneği aracılığıyla npm'den de yükleyebilirsiniz.
