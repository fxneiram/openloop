...
export const OPENLOOP_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const OPENLOOP_MODE_KEY = "openloop.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(OPENLOOP_MODE_KEY, OPENLOOP_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(OPENLOOP_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(OPENLOOP_MODE_KEY, stack.at(-1)?.mode ?? OPENLOOP_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? OPENLOOP_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(OPENLOOP_MODE_KEY, undefined)
