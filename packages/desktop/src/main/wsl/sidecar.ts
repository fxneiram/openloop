...
export async function spawnWslSidecar(
  ...
  const openloop = await resolveWslOpencode(distro)
  if (!openloop) throw new Error(`OpenLoop is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "openloop"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export OPENLOOP_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export OPENLOOP_CLIENT=desktop",
    `export OPENLOOP_SERVER_USERNAME=${shellEscape(username)}`,
    `export OPENLOOP_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(openloop)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
