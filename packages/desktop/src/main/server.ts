...
const SIDECAR_SERVICE_NAME = "openloop server"
...
export function preferAppEnv(userDataPath: string) {
  ...
  Object.assign(process.env, {
    ...shellEnv,
    OPENLOOP_EXPERIMENTAL_ICON_DISCOVERY: "true",
    OPENLOOP_EXPERIMENTAL_FILEWATCHER: "true",
...
export async function checkHealth(url: string, password?: string | null): Promise<boolean> {
  ...
  if (password) {
    const auth = Buffer.from(`openloop:${password}`).toString("base64")
